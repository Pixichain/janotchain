...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if tx.IsJanotsApplyTransaction() {
		...
		if err := core.ValidateJanotsApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
	...
	if tx.IsJanoTApplyTransaction() {
		copyState := pool.currentState(ctx).Copy()
		if err := core.ValidateJanoTApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
