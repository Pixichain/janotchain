...
type Backend interface {
	...
	JanotxService() *janot.JanotX
	LendingService() *janotlending.Lending
	...
	GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, janotState *tradingstate.TradingStateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error)
	...
	return []rpc.API{
		{
			...
			Namespace: "janot",
			Version:   "1.0",
			Service:   NewPublicJanotXTransactionPoolAPI(apiBackend, nonceLock),
			Public:    true,
		}, {
			Namespace: "txpool",
			Version:   "1.0",
