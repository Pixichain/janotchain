...
func VerifyBalance(statedb *state.StateDB, janotStateDb *TradingStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken().String() != common.JanetaNativeAddress {
		quotePrice = janotStateDb.GetLastPrice(GetTradingOrderBookHash(order.QuoteToken(), common.HexToAddress(common.JanetaNativeAddress)))
		log.Debug("TryGet quotePrice QuoteToken/JANETA", "quotePrice", quotePrice)
		if quotePrice == nil || quotePrice.Sign() == 0 {
			inversePrice := janotStateDb.GetLastPrice(GetTradingOrderBookHash(common.HexToAddress(common.JanetaNativeAddress), order.QuoteToken()))
			...
			if inversePrice != nil && inversePrice.Sign() > 0 {
				...
				log.Debug("TryGet quotePrice after get inversePrice JANETA/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteDecimal)
			}
		}
	} else {
