...
const DefaultFeeRate = 10 // 10 / JanoTBaseFee = 10 / 10000 = 0.1%
...
func GetSettleBalance(quotePrice *big.Int, takerSide string, takerFeeRate *big.Int, baseToken, quoteToken common.Address, makerPrice *big.Int, makerFeeRate *big.Int, baseTokenDecimal *big.Int, quoteTokenDecimal *big.Int, quantityToTrade *big.Int) (*SettleBalance, error) {
	...
	if takerSide == Bid {
		...
		if quoteToken.String() != common.JanetaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
			// defaultFeeInJANETA
			defaultFeeInJANETA := new(big.Int).Mul(defaultFee, quotePrice)
			defaultFeeInJANETA = new(big.Int).Div(defaultFeeInJANETA, quoteTokenDecimal)
			...
			if (exMakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInJANETA.Cmp(common.RelayerFee) <= 0 {
				log.Debug("makerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "quotePrice", quotePrice, "defaultFeeInJANETA", defaultFeeInJANETA)
				return result, ErrQuantityTradeTooSmall
			}
			...
			if (exTakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInJANETA.Cmp(common.RelayerFee) <= 0 {
				log.Debug("takerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "quotePrice", quotePrice, "defaultFeeInJANETA", defaultFeeInJANETA)
				return result, ErrQuantityTradeTooSmall
			}
		} else if quoteToken.String() == common.JanetaNativeAddress {
		...
		if quoteToken.String() != common.JanetaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
			...
			if (exMakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInJANETA.Cmp(common.RelayerFee) <= 0 {
				log.Debug("makerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "quotePrice", quotePrice, "defaultMakerFeeInJANETA", defaultFeeInJANETA)
				return result, ErrQuantityTradeTooSmall
			}
			exTakerReceivedFee := new(big.Int).Mul(takerFee, quotePrice)
			exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, quoteTokenDecimal)
