...
type JanoTManagedState struct {
	*TradingStateDB
	mu        sync.RWMutex
	exchanges map[common.Hash]*exchanges
}

// JanoTManagedState returns a new managed state with the statedb as it's backing layer
func ManageState(statedb *TradingStateDB) *JanoTManagedState {
	return &JanoTManagedState{
...
func (ms *JanoTManagedState) SetState(statedb *TradingStateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *JanoTManagedState) RemoveNonce(addr common.Hash, n uint64) {
...
func (ms *JanoTManagedState) NewNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *JanoTManagedState) GetNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if ms.hasAccount(addr) {
		account := ms.getAccount(addr)
...
func (ms *JanoTManagedState) SetNonce(addr common.Hash, nonce uint64) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *JanoTManagedState) HasAccount(addr common.Hash) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *JanoTManagedState) getAccount(addr common.Hash) *exchanges {
