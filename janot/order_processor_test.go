...
import (
	...
	"github.com/janotchain/janotchain/core/rawdb"
...
func Test_getCancelFee(t *testing.T) {
	...
	janot.SetTokenDecimal(testTokenA, common.BasePrice)
	// tokenB has decimal 10^8
	tokenBDecimal := new(big.Int).Exp(big.NewInt(10), big.NewInt(8), nil)
	janot.SetTokenDecimal(testTokenB, tokenBDecimal)

	// set tokenAPrice = 1 JANETA
	tradingStateDb.SetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(testTokenA, common.HexToAddress(common.JanetaNativeAddress)), common.BasePrice)
	// set tokenBPrice = 1 JANETA
	tradingStateDb.SetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(common.HexToAddress(common.JanetaNativeAddress), testTokenB), tokenBDecimal)
	...
	tests := []struct {
		...
		// zero fee test: SELL
		{
			"TokenA/JANETA zero fee test: SELL",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  testTokenA,
					QuoteToken: common.HexToAddress(common.JanetaNativeAddress),
					Quantity:   new(big.Int).SetUint64(10000),
					Side:       tradingstate.Ask,
				},
			},
		...
		// zero fee test: BUY
		{
			"TokenA/JANETA zero fee test: BUY",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  testTokenA,
		...
		// test getCancelFee: SELL
		{
			"TokenA/JANETA test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.JanetaNativeAddress),
		...
		// test getCancelFee:: BUY
		{
			"TokenA/JANETA test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
		...
		// zero fee test: SELL
		{
			"JANETA/TokenA zero fee test: SELL",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.JanetaNativeAddress),
		...
		// zero fee test: BUY
		{
			"JANETA/TokenA zero fee test: BUY",
			CancelFeeArg{
				feeRate: common.Big0,
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.JanetaNativeAddress),
		...
		// test getCancelFee: SELL
		{
			"JANETA/TokenA test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.JanetaNativeAddress),
		...
		// test getCancelFee:: BUY
		{
			"JANETA/TokenA test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, _ := janot.getCancelFee(nil, nil, tradingStateDb, tt.args.order, tt.args.feeRate); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("getCancelFee() = %v, quantity %v", got, tt.want)
			}
		})
	}

	// testcase: can't get price of token in JANETA
	testTokenC := common.HexToAddress("0x1200000000000000000000000000000000000004")
	janot.SetTokenDecimal(testTokenC, big.NewInt(1))
	...
	if fee, _ := janot.getCancelFee(nil, nil, tradingStateDb, tokenCOrder.order, tokenCOrder.feeRate); fee != nil && fee.Sign() != 0 {
	...
	janot.SetTokenDecimal(testTokenD, big.NewInt(0))
	...
	if fee, _ := janot.getCancelFee(nil, nil, tradingStateDb, tokenDOrder.order, tokenDOrder.feeRate); fee != nil && fee.Sign() != 0 {
		t.Errorf("getCancelFee() = %v, want %v", fee, common.Big0)
	}
