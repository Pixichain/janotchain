...
func New(janot *janot.JanoT) *Lending {
	...
	lending.StateCache = lendingstate.NewDatabase(janot.GetLevelDB())
	lending.janot = janot
	return lending
}

func (l *Lending) GetLevelDB() janotDAO.JanoTDAO {
	return l.janot.GetLevelDB()
}

func (l *Lending) GetMongoDB() janotDAO.JanoTDAO {
	return l.janot.GetMongoDB()
}
	...
	return []rpc.API{
		{
			...
			Service:   NewPublicJanoTLendingAPI(l),
			Public:    true,
		},
	}
}
...
func (l *Lending) RollbackLendingData(txhash common.Hash) error {
	...
	if items != nil {
		for _, item := range items.([]*lendingstate.LendingItem) {
			c, ok := l.lendingItemHistory.Get(txhash)
			log.Debug("janotlending reorg: rollback lendingItem", "txhash", txhash.Hex(), "item", lendingstate.ToJSON(item), "lendingItemHistory", c)
			if !ok {
				log.Debug("janotlending reorg: remove item due to no lendingItemHistory", "item", lendingstate.ToJSON(item))
			...
			if (lendingItemHistory == lendingstate.LendingItemHistoryItem{}) {
				log.Debug("janotlending reorg: remove item due to empty lendingItemHistory", "item", lendingstate.ToJSON(item))
				if err := db.DeleteObject(item.Hash, &lendingstate.LendingItem{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingItem. Err: %v . Item: %s", err.Error(), lendingstate.ToJSON(item))
				}
				continue
			...
			log.Debug("janotlending reorg: update item to the last lendingItemHistory", "item", lendingstate.ToJSON(item), "lendingItemHistory", lendingItemHistory)
	...
	if items != nil {
		for _, trade := range items.([]*lendingstate.LendingTrade) {
			c, ok := l.lendingTradeHistory.Get(txhash)
			log.Debug("janotlending reorg: rollback LendingTrade", "txhash", txhash.Hex(), "trade", lendingstate.ToJSON(trade), "LendingTradeHistory", c)
			if !ok {
				log.Debug("janotlending reorg: remove trade due to no LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
			...
			if (lendingTradeHistoryItem == lendingstate.LendingTradeHistoryItem{}) {
				log.Debug("janotlending reorg: remove trade due to empty LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
				if err := db.DeleteObject(trade.Hash, &lendingstate.LendingTrade{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingTrade. Err: %v . Trade: %s", err.Error(), lendingstate.ToJSON(trade))
				}
				continue
			...
			log.Debug("janotlending reorg: update trade to the last lendingTradeHistoryItem", "trade", lendingstate.ToJSON(trade), "lendingTradeHistoryItem", lendingTradeHistoryItem)
			if err := db.PutObject(trade.Hash, trade); err != nil {
