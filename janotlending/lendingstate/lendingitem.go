...
func VerifyBalance(isJanoTLendingFork bool, statedb *state.StateDB, lendingStateDb *LendingStateDB,
	orderType, side, status string, userAddress, relayer, lendingToken, collateralToken common.Address,
	quantity, lendingTokenDecimal, collateralTokenDecimal, lendTokenJANETAPrice, collateralPrice *big.Int,
	...
	case Market, Limit:
		...
		case Investing:
			...
			case LendingStatusNew:
				...
				if lendTokenJANETAPrice != nil && lendTokenJANETAPrice.Sign() > 0 {
					...
					defaultFeeInJANETA := common.Big0
					if lendingToken.String() != common.TomoNativeAddress {
						defaultFeeInJANETA = new(big.Int).Mul(defaultFee, lendTokenJANETAPrice)
						defaultFeeInJANETA = new(big.Int).Div(defaultFeeInJANETA, lendingTokenDecimal)
					} else {
						defaultFeeInJANETA = defaultFee
					}
					if defaultFeeInJANETA.Cmp(common.RelayerLendingFee) <= 0 {
		...
		case Borrowing:
			...
			case LendingStatusNew:
				...
				settleBalanceResult, err := GetSettleBalance(isJanoTLendingFork, Borrowing, lendTokenJANETAPrice, collateralPrice, depositRate, borrowingFeeRate, lendingToken, collateralToken, lendingTokenDecimal, collateralTokenDecimal, quantity)
				if err != nil {
					return err
				}
