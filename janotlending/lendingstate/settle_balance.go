...
const DefaultFeeRate = 100 // 100 / JanoTBaseFee = 100 / 10000 = 1%
var (
...
func GetSettleBalance(isJanoTLendingFork bool,
	takerSide string,
	...
	defaultFee = new(big.Int).Div(defaultFee, common.JanoTBaseFee)

	var result *LendingSettleBalance
	//result = map[common.Address]map[string]interface{}{}

	if !isJanoTLendingFork {
		if takerSide == Borrowing {
			...
			takerFee = new(big.Int).Div(takerFee, common.JanoTBaseFee)
			...
			if lendingToken.String() != common.TomoNativeAddress && lendTokenJANETAPrice != nil && lendTokenJANETAPrice.Cmp(common.Big0) > 0 {
				exTakerReceivedFee := new(big.Int).Mul(takerFee, lendTokenJANETAPrice)
				exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, lendTokenDecimal)

				defaultFeeInJANETA := new(big.Int).Mul(defaultFee, lendTokenJANETAPrice)
				defaultFeeInJANETA = new(big.Int).Div(defaultFeeInJANETA, lendTokenDecimal)

				if (exTakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInJANETA.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("takerFee too small", "quantityToLend", quantityToLend, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInJANETA", defaultFeeInJANETA)
					return result, ErrQuantityTradeTooSmall
				}
			...
			if lendingToken.String() != common.TomoNativeAddress && lendTokenJANETAPrice != nil && lendTokenJANETAPrice.Cmp(common.Big0) > 0 {
				exMakerReceivedFee := new(big.Int).Mul(makerFee, lendTokenJANETAPrice)
				exMakerReceivedFee = new(big.Int).Div(exMakerReceivedFee, lendTokenDecimal)

				defaultFeeInJANETA := new(big.Int).Mul(defaultFee, lendTokenJANETAPrice)
				defaultFeeInJANETA = new(big.Int).Div(defaultFeeInJANETA, lendTokenDecimal)

				if (exMakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInJANETA.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("makerFee too small", "quantityToLend", quantityToLend, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInJANETA", defaultFeeInJANETA)
					return result, ErrQuantityTradeTooSmall
				}
			} else if lendingToken.String() == common.TomoNativeAddress {
		...
		borrowFee = new(big.Int).Div(borrowFee, common.JanoTBaseFee)
		...
		if lendingToken.String() != common.TomoNativeAddress && lendTokenJANETAPrice != nil && lendTokenJANETAPrice.Cmp(common.Big0) > 0 {
			...
			exReceivedFee := new(big.Int).Mul(borrowFee, lendTokenJANETAPrice)
			exReceivedFee = new(big.Int).Div(exReceivedFee, lendTokenDecimal)

			defaultFeeInJANETA := new(big.Int).Mul(defaultFee, lendTokenJANETAPrice)
			defaultFeeInJANETA = new(big.Int).Div(defaultFeeInJANETA, lendTokenDecimal)

			if (exReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exReceivedFee.Sign() > 0) || defaultFeeInJANETA.Cmp(common.RelayerLendingFee) <= 0 {
				log.Debug("takerFee too small", "quantityToLend", quantityToLend, "borrowFee", borrowFee, "exReceivedFee", exReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInJANETA", defaultFeeInJANETA)
				return result, ErrQuantityTradeTooSmall
			}
		} else if lendingToken.String() == common.TomoNativeAddress {
