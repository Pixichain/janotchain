	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := VerifyBalance(true,
				...
				EtherToWei(big.NewInt(1)),
				EtherToWei(big.NewInt(2)),    // JANETA price: 0.5 USD => USD/JANETA = 2
...
func sendOrder(nonce uint64) {
	...
	err = rpcClient.Call(&result, "janot_sendLending", item)
	fmt.Println("sendLendingitem", "nonce", item.AccountNonce)
	if err != nil {
		fmt.Println("rpcClient.Call janot_sendLending failed", "err", err)
		os.Exit(1)
	}
}
