...
func (l *Lending) processOrderList(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, lendingStateDB *lendingstate.LendingStateDB, tradingStateDb *tradingstate.TradingStateDB, side string, lendingOrderBook common.Hash, Interest *big.Int, quantityStillToTrade *big.Int, order *lendingstate.LendingItem) (*big.Int, []*lendingstate.LendingTrade, []*lendingstate.LendingItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		lendTokenJANETAPrice, collateralPrice, err := l.GetCollateralPrices(header, chain, statedb, tradingStateDb, collateralToken, order.LendingToken)
		if err != nil {
			return nil, nil, nil, err
		}
		if lendTokenJANETAPrice == nil || lendTokenJANETAPrice.Sign() <= 0 {
		...
		tradedQuantity, collateralLockedAmount, rejectMaker, settleBalanceResult, err := l.getLendQuantity(lendTokenJANETAPrice, collateralPrice, depositRate, borrowFee, coinbase, chain, header, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (l *Lending) getLendQuantity(
	...
	LendingTokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, makerOrder.LendingToken)
	...
	collateralTokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, collateralToken)
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		isJanoTLendingFork := chain.Config().IsTIPJanoTLending(header.Number)
		settleBalanceResult, err := lendingstate.GetSettleBalance(isJanoTLendingFork, takerOrder.Side, lendTokenJANETAPrice, collateralPrice, depositRate, borrowFee, lendToken, collateralToken, LendingTokenDecimal, collateralTokenDecimal, quantity)
...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	lendTokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, originOrder.LendingToken)
	...
	if originOrder.Side == lendingstate.Borrowing {
		...
		collateralTokenDecimal, err = l.janot.GetTokenDecimal(chain, statedb, originOrder.CollateralToken)
	...
	if !chain.Config().IsTIPJanoTCancellationFee(header.Number) {
		...
		tokenCancelFee, tokenPriceInJANETA = l.getCancelFee(chain, statedb, tradingStateDb, &originOrder, feeRate)
	}
...
func (l *Lending) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *lendingstate.LendingItem, feeRate *big.Int) (*big.Int, *big.Int) {
	...
	cancelFee, tokenPriceInJANETA := common.Big0, common.Big0
	var err error
	if order.Side == lendingstate.Investing {
		cancelFee, tokenPriceInJANETA, err = l.janot.ConvertJANETAToToken(chain, statedb, tradingStateDb, order.LendingToken, common.RelayerLendingCancelFee)
	} else {
		cancelFee, tokenPriceInJANETA, err = l.janot.ConvertJANETAToToken(chain, statedb, tradingStateDb, order.CollateralToken, common.RelayerLendingCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
...
func (l *Lending) GetMediumTradePriceBeforeEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
	...
	if price != nil && price.Sign() > 0 {
		...
		if inversePrice != nil && inversePrice.Sign() > 0 {
			quoteTokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, quoteToken)
			if err != nil || quoteTokenDecimal.Sign() == 0 {
				return nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", quoteToken.String(), err)
			}
			baseTokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, baseToken)
...
//- Have direct pair in JanoT: lendToken/CollateralToken or CollateralToken/LendToken
//- Have pairs with JANETA:
//-  lendToken/JANETA and CollateralToken/JANETA
//-  JANETA/lendToken and JANETA/CollateralToken
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	// lendTokenJANETAPrice: price of ticker lendToken/JANETA
	// collateralJANETAPrice: price of ticker collateralToken/JANETA
	...
	lendTokenJANETAPrice, err := l.GetJANETABasePrices(header, chain, statedb, tradingStateDb, lendingToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralPriceUpdatedFromContract {
		...
		return lendTokenJANETAPrice, collateralPriceFromContract, nil
	}
	lendingTokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, lendingToken)
	...
	if inverseCollateralPriceUpdatedFromContract {
		...
		return lendTokenJANETAPrice, collateralPrice, nil
	}
	// if contract doesn't provide any price information
	// getting price from pair in janot
	...
	if lastAveragePrice != nil && lastAveragePrice.Sign() > 0 {
		log.Debug("Getting collateral/lending from direct pair in janot", "lendToken", lendingToken.Hex(), "collateralToken", collateralToken.Hex(), "price", lastAveragePrice)
		return lendTokenJANETAPrice, lastAveragePrice, nil
	}
	collateralJANETAPrice, err := l.GetJANETABasePrices(header, chain, statedb, tradingStateDb, collateralToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralJANETAPrice == nil || lendTokenJANETAPrice == nil {
		return common.Big0, common.Big0, nil
	}
	// Calculate collateral/LendToken price from collateral/JANETA, lendToken/JANETA
	collateralPrice = new(big.Int).Mul(collateralJANETAPrice, lendingTokenDecimal)
	collateralPrice = new(big.Int).Div(collateralPrice, lendTokenJANETAPrice)
	log.Debug("GetCollateralPrices: Calculate collateral/LendToken price from collateral/JANETA, lendToken/JANETA", "collateralPrice", collateralPrice,
		"collateralJANETAPrice", collateralJANETAPrice, "lendingTokenDecimal", lendingTokenDecimal, "lendTokenJANETAPrice", lendTokenJANETAPrice)
	return lendTokenJANETAPrice, collateralPrice, nil
}

func (l *Lending) GetJANETABasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {

	tokenJANETAPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, token, common.HexToAddress(common.TomoNativeAddress))
	tokenJANETAPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().Posv.Epoch == header.Number.Uint64()/chain.Config().Posv.Epoch

	if token == common.HexToAddress(common.TomoNativeAddress) {
		return common.BasePrice, nil
	} else if tokenJANETAPriceUpdatedFromContract {
		// getting lendToken price from contract first
		// otherwise, getting from janot lendToken/JANETA
		log.Debug("Getting token/JANETA price from contract", "price", tokenJANETAPriceFromContract)
		return tokenJANETAPriceFromContract, nil
	} else {
		janotTokenPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, common.HexToAddress(common.TomoNativeAddress), token)
		janotTokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().Posv.Epoch == header.Number.Uint64()/chain.Config().Posv.Epoch
		if janotTokenPriceUpdatedFromContract && janotTokenPriceFromContract != nil && janotTokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from janot lendToken/JANETA
			log.Debug("Getting JANETA/token from contract", "price", janotTokenPriceFromContract)
			tokenDecimal, err := l.janot.GetTokenDecimal(chain, statedb, token)
			...
			tokenTomoPrice = new(big.Int).Div(tokenTomoPrice, janotTokenPriceFromContract)
			return tokenTomoPrice, nil
		}
		tokenJANETAPrice, err := l.GetMediumTradePriceBeforeEpoch(chain, statedb, tradingStateDb, token, common.HexToAddress(common.TomoNativeAddress))
		if err != nil {
			return nil, err
		}
		if tokenJANETAPrice != nil && tokenJANETAPrice.Sign() > 0 {
			log.Debug("Getting token/JANETA from janot", "price", tokenJANETAPrice, "err", err)
			return tokenJANETAPrice, nil
		}
	}
	log.Debug("Can't getting tokenJANETAPrice ", "token", token.Hex())
	return nil, nil
}
...
func (l *Lending) ProcessRepayLendingTrade(header *types.Header, chain consensus.ChainContext, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingstateDB *tradingstate.TradingStateDB, lendingBook common.Hash, lendingTradeId uint64) (trade *lendingstate.LendingTrade, err error) {
	...
	if tokenBalance.Cmp(paymentBalance) < 0 {
		...
		var err error
		if chain.Config().IsTIPJanoTLending(header.Number) {
