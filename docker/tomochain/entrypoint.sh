...
params=""
accountsCount=$(
  janeta account list --datadir $DATA_DIR  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    ...
    89 )
      ...
      params="$params --janeta-testnet --gcmode archive --rpcapi db,eth,net,web3,debug,posv"
      ;;
    90 )
...
# data dir
if [[ ! -d $DATA_DIR/janeta ]]; then
  echo "No blockchain data, creating genesis block."
  janeta init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    janeta  account import ./private_key \
      --datadir $DATA_DIR \
