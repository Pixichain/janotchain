...
var (
	...
	skipNonceDestinationAddress = map[string]bool{
		common.JanotAddr:                         true,
		common.TradingStateAddr:                  true,
		common.JanotLendingAddress:               true,
		common.JanotLendingFinalizedTradeAddress: true,
	}
)
...
func (tx *Transaction) IsTradingTransaction() bool {
	...
	if tx.To().String() != common.JanotAddr {
		return false
	}
...
func (tx *Transaction) IsLendingTransaction() bool {
	...
	if tx.To().String() != common.JanotLendingAddress {
		return false
	}
	return true
}
...
func (tx *Transaction) IsLendingFinalizedTradeTransaction() bool {
	...
	if tx.To().String() != common.JanotLendingFinalizedTradeAddress {
		return false
	}
	return true
}
...
func (tx *Transaction) IsJanotApplyTransaction() bool {
	if tx.To() == nil {
		return false
	}

	addr := common.JanotListingSMC
	if common.IsTestnet {
		addr = common.JanotListingSMCTestNet
	}
	...
	if method != common.JanotApplyMethod {
		return false
	}
...
func (tx *Transaction) IsJanoTsApplyTransaction() bool {
	if tx.To() == nil {
		return false
	}
	...
	method := common.ToHex(tx.Data()[0:4])
	if method != common.JanoTsApplyMethod {
		return false
	}
