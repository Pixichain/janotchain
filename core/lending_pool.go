...
// blockChain_janot add order state
...
func (pool *LendingPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPJanot(pool.chain.CurrentBlock().Number()) || pool.chain.Config().Posv == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().Posv.Epoch {
		return
	}
...
func (pool *LendingPool) validateBalance(cloneStateDb *state.StateDB, cloneLendingStateDb *lendingstate.LendingStateDB, tx *types.LendingTransaction, collateralToken common.Address) error {
	...
	JanotServ := posvEngine.GetJanoTService()
	lendingServ := posvEngine.GetLendingService()
	if JanotServ == nil {
		return fmt.Errorf("janot not found in order validation")
	}
	lendingTokenDecimal, err := JanotServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.LendingToken())
	if err != nil {
	...
	tradingStateDb, err := JanotServ.GetTradingState(pool.chain.CurrentBlock(), author)
	if err != nil {
	...
	// lendTokenJANETAPrice: price of lendingToken in JANETA quote
	var lendTokenJANETAPrice, collateralPrice, collateralTokenDecimal *big.Int
	if collateralToken.String() != lendingstate.EmptyAddress {
		collateralTokenDecimal, err = JanotServ.GetTokenDecimal(pool.chain, cloneStateDb, collateralToken)
		if err != nil {
			return fmt.Errorf("validateOrder: failed to get collateralTokenDecimal. err: %v", err)
		}
		lendTokenJANETAPrice, collateralPrice, err = lendingServ.GetCollateralPrices(pool.chain.CurrentHeader(), pool.chain, cloneStateDb, cloneTradingStateDb, collateralToken, tx.LendingToken())
		if err != nil {
			return err
		}
		if lendTokenJANETAPrice == nil || lendTokenJANETAPrice.Sign() <= 0 || collateralPrice == nil || collateralPrice.Sign() <= 0 {
			log.Debug("ValidateLending: ErrInvalidCollateralPrice", "lendTokenJANETAPrice", lendTokenJANETAPrice, "collateralPrice", collateralPrice)
			return lendingstate.ErrInvalidCollateralPrice
		}
	}
	if lendTokenJANETAPrice == nil || lendTokenJANETAPrice.Sign() == 0 {
		if tx.LendingToken().String() == common.JanetaNativeAddress {
			lendTokenJANETAPrice = common.BasePrice
		} else {
			lendTokenJANETAPrice, err = lendingServ.GetMediumTradePriceBeforeEpoch(pool.chain, cloneStateDb, cloneTradingStateDb, tx.LendingToken(), common.HexToAddress(common.JanetaNativeAddress))
			if err != nil {
				return err
			}
		}
	}
	isJanotLendingFork := pool.chain.Config().IsTIPJanotLending(pool.chain.CurrentHeader().Number)
	if err := lendingstate.VerifyBalance(isJanotLendingFork,
		cloneStateDb,
		cloneLendingStateDb,
		tx.Type(),
		tx.Side(),
...
func (pool *LendingPool) addTx(tx *types.LendingTransaction, local bool) error {
	if !pool.chainconfig.IsTIPJanot(pool.chain.CurrentBlock().Number()) {
		return nil
	}
	tx.CacheHash()
