...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	*/
	...
	if tx.IsJanoTsApplyTransaction() {
		...
		return ValidateJanoTsApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}

	// validate balance slot, token decimal for Janot
	if tx.IsJanotApplyTransaction() {
		copyState := pool.currentState.Copy()
		return ValidateJanotApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}
	return nil
}
