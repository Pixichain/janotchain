...
import (
	...
	"github.com/janotchain/janotchain/common/mclock"
	...
	"github.com/janotchain/janotchain/ethclient"
	"github.com/janotchain/janotchain/ethdb"
	"github.com/janotchain/janotchain/event"
	"github.com/janotchain/janotchain/log"
	"github.com/janotchain/janotchain/metrics"
	"github.com/janotchain/janotchain/params"
	"github.com/janotchain/janotchain/rlp"
	"github.com/janotchain/janotchain/trie"
...
func NewBlockChainEx(db ethdb.Database, janotDb ethdb.JanotDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addJanotDb(janotDb)
	}
...
func (bc *BlockChain) addJanotDb(janotDb ethdb.JanotDatabase) {
	bc.janotDb = janotDb
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		if ok {
			tradingService := engine.GetJanoTService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPJanot(currentBlock.Number()) && bc.chainConfig.Posv != nil && currentBlock.NumberU64() > bc.chainConfig.Posv.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*posv.Posv)
	if ok {
		JanoTService := engine.GetJanoTService()
		if bc.Config().IsTIPJanot(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && JanoTService != nil {
			...
			if err == nil {
				return janotState, nil
			} else {
				return nil, err
			}
		}
	}
	return nil, errors.New("Get janot state fail")
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPJanot(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetJanoTService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPJanot((*head).Number()) && bc.chainConfig.Posv != nil && (*head).NumberU64() > bc.chainConfig.Posv.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	if bc.Config().IsTIPJanot(block.Number()) && bc.chainConfig.Posv != nil && engine != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch {
		tradingService := engine.GetJanoTService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) SaveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPJanot(bc.CurrentBlock().Number()) && bc.chainConfig.Posv != nil && bc.CurrentBlock().NumberU64() > bc.chainConfig.Posv.Epoch && engine != nil {
			tradingService = engine.GetJanoTService()
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPJanot(recent.Number()) && bc.chainConfig.Posv != nil && recent.NumberU64() > bc.chainConfig.Posv.Epoch && engine != nil {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	var lendingTrieDb *trie.Database
	var lendingService posv.LendingService
	if bc.Config().IsTIPJanot(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && engine != nil {
		tradingService = engine.GetJanoTService()
		if tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
			...
			if tradingService != nil && lendingService != nil {
				...
				if tradingState != nil && tradingService != nil {
					...
					if gotRoot != expectRoot {
						err = fmt.Errorf("invalid janot trading state merke trie got : %s , expect : %s ,parent : %s", gotRoot.Hex(), expectRoot.Hex(), parentRoot.Hex())
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
					log.Debug("Janot Trading State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
				if lendingState != nil && tradingState != nil {
					...
					log.Debug("Janot Lending State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
			}
		}
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPJanot(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if bc.chainConfig.IsTIPJanot(commonBlock.Number()) && bc.chainConfig.Posv != nil && commonBlock.NumberU64() > bc.chainConfig.Posv.Epoch {
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	JanoTService := engine.GetJanoTService()
	if JanoTService == nil || !JanoTService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractTradingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			...
			if err := JanoTService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
...
func (bc *BlockChain) reorgTxMatches(deletedTxs types.Transactions, newChain types.Blocks) {
	...
	for _, deletedTx := range deletedTxs {
		if deletedTx.IsTradingTransaction() {
			...
			if err := JanoTService.RollbackReorgTxMatch(deletedTx.Hash()); err != nil {
