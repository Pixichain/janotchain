...
func getNonce(t *testing.T, userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "janot_getOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func testSendOrderJANETAUSD(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func testSendOrderJANETABTC(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func TestFilled(t *testing.T) {
	...
	//testSendOrderJANETAUSD(t, new(big.Int).Mul(big.NewInt(1000000000000000000), big.NewInt(5000)), BTCUSDPrice, "BUY", "NEW", 0)
	//ETH/BTC
	...
	JANETABTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	time.Sleep(2 * time.Second)
	testSendOrderJANETABTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETABTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderJANETABTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETABTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderJANETABTC(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), JANETABTCPrice, "SELL", "NEW", 0)

	JANETAUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(6)) // 0.6
	time.Sleep(2 * time.Second)
	testSendOrderJANETAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETAUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderJANETAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETAUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderJANETAUSD(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), JANETAUSDPrice, "SELL", "NEW", 0)

}

func TestX10Filled(t *testing.T) {
	JANETAUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(60)) // 6
	time.Sleep(2 * time.Second)
	testSendOrderJANETAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETAUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderJANETAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETAUSDPrice, "BUY", "NEW", 0)
...
func TestCancelOrder(t *testing.T) {
	JANETABTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETABTCPrice, "BUY", "NEW", 0)
	time.Sleep(5 * time.Second)
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), JANETABTCPrice, "BUY", "CANCELLED", 3)
	time.Sleep(5 * time.Second)
