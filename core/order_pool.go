...
type blockChainJanot interface {
...
type OrderPool struct {
	...
	pendingState      *tradingstate.JanotManagedState // Pending state tracking virtual nonces
...
func NewOrderPool(chainconfig *params.ChainConfig, chain blockChainJanot) *OrderPool {
...
func (pool *OrderPool) State() *tradingstate.JanotManagedState {
	pool.mu.RLock()
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	cloneJanotStateDb := pool.currentOrderState.Copy()
	...
	if !tx.IsCancelledOrder() {
		...
		if orderType == OrderTypeLimit {
			...
			baseDecimal, err := JanotServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.BaseToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get baseDecimal. err: %v", err)
			}
			quoteDecimal, err := JanotServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.QuoteToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get quoteDecimal. err: %v", err)
			}
			if err := tradingstate.VerifyBalance(cloneStateDb, cloneJanotStateDb, tx, baseDecimal, quoteDecimal); err != nil {
				return err
			}
		}
	...
	if !tx.IsCancelledOrder() {
		...
		originOrder := cloneJanotStateDb.GetOrder(tradingstate.GetTradingOrderBookHash(tx.BaseToken(), tx.QuoteToken()), common.BigToHash(new(big.Int).SetUint64(tx.OrderID())))
