...
import (
	...
	"github.com/janotchain/janotchain/crypto/sha3"
...
func getLendingNonce(userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "janot_getLendingOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func TestSendLending(t *testing.T) {
	...
	for true {
		...
		// lendToken: USD, collateral: JANETA
		// amount 1000 USD
		testSendLending(key, nonce, USDAddress, common.Address{}, new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, USDAddress, common.HexToAddress(common.JanetaNativeAddress), new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)

		// lendToken: BTC, collateral: JANETA
		// amount 1 BTC
		...
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, BTCAddress, common.HexToAddress(common.JanetaNativeAddress), new(big.Int).Mul(_1E18, big.NewInt(1)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		...
		// lendToken: JANETA, collateral: BTC
		// amount 1000 JANETA
		testSendLending(key, nonce, common.HexToAddress(common.JanetaNativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.JanetaNativeAddress), BTCAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)

		// lendToken: JANETA, collateral: ETH
		// amount 1000 JANETA
		testSendLending(key, nonce, common.HexToAddress(common.JanetaNativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.JanetaNativeAddress), ETHAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
	}
}
