...
import (
	...
	"github.com/janetachain/janetachain/contracts/trc21issuer"
	"github.com/janetachain/janetachain/contracts/trc21issuer/simulation"
...
func airDropTokenToAccountNoJaneta() {
...
func testTransferTRC21TokenWithAccountNoJaneta() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have janeta
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoJaneta()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoJaneta()
		fmt.Println("Finish transfer trc21 token with a account no janeta")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no janeta")
	}
