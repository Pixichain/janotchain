...
contract RelayerRegistration {
    ...
    address constant private janetaNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractJanoTListing private JanotListing;
    ...
    constructor (address janotListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        JanotListing = AbstractJanoTListing(janotListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with janeta(x/JANETA)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 JANETA is required for a deposit request");
    ...
    function indexOf(address[] memory janetaPair, address target) internal pure returns (bool){
        for (uint i = 0; i < janetaPair.length; i ++) {
            if (janetaPair[i] == target) {
                return true;
            }
        }
        return false;
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory janetaPairs = new address[](fromTokens.length);
        address[] memory nonJanetaPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = JanotListing.getTokenStatus(toTokens[i]) || (toTokens[i] == janetaNative);
            b = b && (JanotListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == janetaNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == janetaNative) {
                janetaPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == janetaNative) {
                    janetaPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonJanetaPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(janetaPairs, nonJanetaPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory janetaPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = JanotListing.getTokenStatus(toToken) || (toToken == janetaNative);
        b = b && (JanotListing.getTokenStatus(fromToken) || fromToken == janetaNative);
        if (!b) {
            return false;
        }

        if (fromToken == janetaNative || toToken == janetaNative) {
            return true;
        }

        // get tokens that paired with janeta
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == janetaNative) {
                janetaPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == janetaNative) {
                    janetaPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(janetaPairs, toToken)) {
            return false;
        }
        return true;
    }
