...
import (
	...
	simulation "github.com/janetachain/janetachain/contracts/janot/testnet"
...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := janot.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	//init JanoT Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	janotListtingAddr, janotListing, err := janot.DeployJanoTListing(auth, client)
	if err != nil {
		log.Fatal("DeployJanoTListing", err)
	}
	janotListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> janot listing address", janotListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : janot listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := janot.DeployRelayerRegistration(auth, client, janotListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := janot.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, janotListtingAddr)
	if err != nil {
	...
	applyJanotListing(janotListing, tokenList, currentNonce)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddCollateral(simulation.JANETANative, simulation.CollateralDepositRate, simulation.CollateralLiquidationRate, simulation.CollateralRecallRate)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddBaseToken(simulation.JANETANative)
	if err != nil {
		log.Fatal("Lending add base token JANETA", err)
	}
	...
	relayerRegistration, err = janot.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	/*
		for _, token := range tokenList {
			...
			toTokens = append(toTokens, simulation.JANETANative)
		}
	*/

	// JANETA/BTC
	fromTokens = append(fromTokens, simulation.JANETANative)
	...
	fmt.Println("wait 2s to apply token to list janot")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = janot.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	...
	// JANETA 1 min
	baseTokens = append(baseTokens, simulation.JANETANative)
	terms = append(terms, big.NewInt(60))
	collaterals = append(collaterals, common.HexToAddress("0x0"))
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		if tokenName == "ADA" {
			d = 0
			tokenCap = new(big.Int).Div(simulation.TRC21TokenCap, simulation.BaseJANETA)
		}
		if tokenName == "USDT" {
			d = 6
		...
		tokenAddr, _, err := janot.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *janot.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyJanotListing(janotListing *janot.JanoTListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		janotListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		janotListing.TransactOpts.Value = simulation.JanotListingFee
		_, err := janotListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("janotListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyJanotListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := janot.NewTRC21(auth, token["address"].(common.Address), client)
