...
import (
	...
	"github.com/janetachain/janetachain/contracts/validator/contract"
	"math/big"
)

type Validator struct {
	*contract.JanetaValidatorSession
...
func NewValidator(transactOpts *bind.TransactOpts, contractAddr common.Address, contractBackend bind.ContractBackend) (*Validator, error) {
	validator, err := contract.NewJanetaValidator(contractAddr, contractBackend)
	if err != nil {
		return nil, err
	}
...
func DeployValidator(transactOpts *bind.TransactOpts, contractBackend bind.ContractBackend, validatorAddress []common.Address, caps []*big.Int, ownerAddress common.Address) (common.Address, *Validator, error) {
	...
	// Deposit 50K JANETA
	// Min Voter Cap 10 JANETA
	// 150 masternodes
	// Candidate Delay Withdraw 30 days = 1296000 blocks
	// Voter Delay Withdraw 2 days = 86400 blocks
	validatorAddr, _, _, err := contract.DeployJanetaValidator(transactOpts, contractBackend, validatorAddress, caps, ownerAddress, minDeposit, minVoterCap, big.NewInt(150), big.NewInt(1296000), big.NewInt(86400))
	if err != nil {
