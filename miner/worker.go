...
import (
	...
	"github.com/janotchain/janotchain/consensus/misc"
	"github.com/janotchain/janotchain/consensus/posv"
	"github.com/janotchain/janotchain/contracts"
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	var janotState *tradingstate.TradingStateDB
	var lendingState *lendingstate.LendingStateDB
	if self.config.Posv != nil {
		JanoT := self.eth.GetJanoT()
		janotState, err = JanoT.GetTradingState(parent, author)
		if err != nil {
			log.Error("Failed to get janot state ", "number", parent.Number(), "err", err)
			return err
		}
		lending := self.eth.GetJanoTLending()
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.Posv != nil && self.chain.Config().IsTIPJanoT(header.Number) {
			JanoT := self.eth.GetJanoT()
			JanoTLending := self.eth.GetJanoTLending()
			if JanoT != nil && header.Number.Uint64() > self.config.Posv.Epoch {
				if header.Number.Uint64()%self.config.Posv.Epoch == 0 {
					err := JanoT.UpdateMediumPriceBeforeEpoch(header.Number.Uint64()/self.config.Posv.Epoch, work.tradingState, work.state)
					if err != nil {
				...
				//https://github.com/janotchain/janotchain-v1/pull/416
				if header.Number.Uint64()%self.config.Posv.Epoch != 0 {
					...
					tradingTxMatches, tradingMatchingResults = JanoT.ProcessOrderPending(header, self.coinbase, self.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = JanoTLending.ProcessOrderPending(header, self.coinbase, self.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%self.config.Posv.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = JanoTLending.ProcessLiquidationData(header, self.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(tradingTxMatches) > 0 {
					...
					tx := types.NewTransaction(nonce, common.HexToAddress(common.JanoTAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					lendingTx := types.NewTransaction(nonce, common.HexToAddress(common.JanoTLendingAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.HexToAddress(common.JanoTLendingFinalizedTradeAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
		...
		JanoTStateRoot := work.tradingState.IntermediateRoot()
		LendingStateRoot := work.lendingState.IntermediateRoot()
		txData := append(JanoTStateRoot.Bytes(), LendingStateRoot.Bytes()...)
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		// validate balance slot, token decimal for JanoT
		if tx.IsJanoTApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateJanoTApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("JanoTApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
